...
class TabularPermissionsWidget(SelectMultiple):

    def __init__(self, *args, **kwargs):
        super(TabularPermissionsWidget, self).__init__(*args, **kwargs)
        self.managed_perms = []
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        user_permissions = Permission.objects.filter(id__in=value or []).values_list('id', flat=True).prefetch_related(
            'content_type')
        all_perms = Permission.objects.all().values('id', 'codename', 'content_type_id').order_by(
            'codename').prefetch_related('content_type')
        excluded_perms = set([])
        codename_id_map = {}
        for p in all_perms:
